...
void BlockQueuePathIntegrator::updateUniforms(const RenderStatus& status)
{
	...
	mShader->set1i("uQueueCapacity", StreamingBlockSize);

	mImageClearShader->setVec2i("uTexSize", size);
}

void BlockQueuePathIntegrator::init(Scene* scene, int width, int height, PipelinePtr ctx)
{
	mShader = Shader::createFromText("pt_block_queue.glsl", { PrimaryBlockSizeX, PrimaryBlockSizeY, 1 },
...
void BlockQueuePathIntegrator::renderOnePass()
{
	mFreeCounter++;
	if (mShouldReset)
	{
...
void BlockQueuePathIntegrator::reset(const RenderStatus& status)
{
...
void BlockQueuePathIntegrator::renderSettingsGUI()
{
...
void BlockQueuePathIntegrator::renderProgressGUI()
{
	if (mParam.finiteSample)
